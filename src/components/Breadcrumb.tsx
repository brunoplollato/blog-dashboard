...
const Breadcrumb = ({ items }: Breadcrumb) => {
  return (
    <nav className="my-2 mb-10 drop-shadow-sm">
      <ul className="flex">
        {items.map(({ text, link }, index) => (
          <li key={index} className="flex items-center">
            <span
              className={`
                text-gray-700 text-sm bg-white border border-r-0 border-l-0 border-slate-300 px-2 py-1 font-bold
                ${index === 0 ? 'border-l pl-4 rounded-l-md' : 'border-r-0'}
              `}
            >
